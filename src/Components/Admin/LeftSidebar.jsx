...
function LeftSidebar({setLeftSidebarOpen,leftSidebarOpen}) {
  ...
  <div className=' mt-[2rem] border-t border-gray-600 py-[1rem] pl-[1rem]  border-b'>
   ...
   <ul className='pl-[1rem] text-gray-500'>
    ...
    <button className='border-[2px] text-gray-400 py-[5px] rounded-md border-gray-400 hover:border-gray-300 hover:text-gray-300 flex justify-center items-center' onClick={()=>{setOpenCreateQuizModal(!openCreateQuizModal)}}>Generate a quiz</button>
    </div>
  </div>
  </motion.div>}
  {/* {<QuizModal openCreateQuizModal={openCreateQuizModal} setOpenCreateQuizModal={setOpenCreateQuizModal} />} */}
   </AnimatePresence>
  )
}
