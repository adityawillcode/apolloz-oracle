...
function AdminNavbar({loginData,wantLogout,selectedGroup,setSelectedGroup}) {
 ...
 const openBar=(fun)=>{
  ...
  return (
    <div className='h-[3rem] bg-black/40   items-center  w-screen text-white border-b-[1px] border-gray-500 flex justify-between px-[1.7rem]' >
     <div className='text-gray-400 ' style={{fontFamily:'Nunito'}} data-tooltip-id='adminText' >{loginData.name}</div>
        <div className='lg:flex h-full relative justify-center items-center hidden gap-[22px] cursor-pointer text-gray-400'><FaUserCircle  size={22} />  <IoIosSettings onClick={()=>{openBar(()=>{setShowSettings(!showSettings)})}} size={27} />  
          ...
          {
            showSettings &&
             <motion.div initial={{y:'-5rem',opacity:0}} animate={{y:'0rem',opacity:1}} exit={{opacity:0}} transition={{duration:'0.2'}} className=' select-none  text-white absolute top-[100%] w-[10rem] bg-black/40 flex flex-col rounded-md justify-center items-center '>
